...
const interactionLogin = (provider) => async (ctx) => {
  const {
    prompt: { name },
  } = await provider.interactionDetails(ctx.req, ctx.res)
  assert.equal(name, 'login')

  const account = await Account.findAccountByLogin(ctx.request.body)
...
const interactionConfirmation = (provider) => async (ctx) => {
...
const interactionAbort = (provider) => async (ctx) => {
  const result = {
    error: 'access_denied',
